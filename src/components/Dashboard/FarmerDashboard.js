...
import { Agriculture, TrendingUp, ShoppingCart, Assessment } from '@mui/icons-material';
  ...
  const loadDashboardData = async () => {
    try {
      ...
      if (storedOrders && storedOrders.length > 0) {
        ...
        try {
          const [fieldsResponse, productsResponse] = await Promise.all([
      ...
      const [fieldsResponse, productsResponse, ordersResponse] = await Promise.all([
        fieldService.getFields(),
        productService.getProducts(),
      ...
      <Grid container spacing={3} sx={{ mb: 4 }}>
        ...
        <Grid item xs={12} md={3}>
          <Card className="farmville-card">
            <CardContent>
              <Box sx={{ display: 'flex', alignItems: 'center', mb: 2 }}>
                <Store sx={{ mr: 1, color: 'secondary.main' }} />
