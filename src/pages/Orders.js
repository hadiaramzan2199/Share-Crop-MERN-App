    ...
    try {
      ...
      if (storedOrders && storedOrders.length > 0) {
        ...
        const formattedOrders = storedOrders.map(order => ({
          ...
          buyer_name: order.buyerName || 'Current User',
          ...
          farm_name: order.farmName || 'Unknown Farm',
          crop_type: order.cropType || order.productName || 'Unknown Crop',
          price_per_unit: order.pricePerKg || order.price || 0,
      ...
      let ordersResponse;
      try {
        ordersResponse = await orderService.getBuyerOrders();
        setOrders(ordersResponse.data.orders || []);
      } catch (apiError) {
        // Fallback to mock service
        ordersResponse = await mockOrderService.getBuyerOrders();
        setOrders(ordersResponse.data.orders || []);
      }
      setError(null);
    } catch (err) {
